...
// Batched-layer isolation test for ConformerLayer::build_graph_batched (T3.2),
// which also exercises the conv module's B>1 pad-masking (T3.1) for the first
// time. It empirically validates the assembled batched conformer layer by
// comparing per-item batched output slices against the scalar
// ConformerLayer::forward reference.
//
// Input source: a real baseline tensor ("l0_attn_in" = [T, d_model] row-major)
// is reused as a deterministic [T,D] layer input, with baseline "pos_emb"
// ([2T-1, d_model] row-major) as the positional encoding. This test validates
// batched==scalar equivalence of the SAME layer on the SAME input; it does NOT
// need to match NeMo's layer output, so any consistent [T,D] input works as
// long as both paths get the same data.
//
...
//   Proves the assembled batched layer is correct under batching.
...
//   exercising the conv module's B>1 pad-masking).
...
int main() {
    ...
    // Deterministic [T, d_model] layer input from the baseline.
    ...
    auto run_batched = [&](int B, const std::vector<int>& valid_len,
        ...
        return pk::run_graph(/*mem_bytes*/0, /*n_threads*/4,
            [&](ggml_context* ctx) -> ggml_tensor* {
                ...
                return layer.build_graph_batched(ctx, xt, T, B, pet, pos_len,
                                                 valid_len, pool);
            }, out);
    };
    ...
    // ---- Case A: equal-length B=2, identical clips ----
    {
        ...
        layer.forward(x, T, pe, pos_len, /*valid_len*/T, ref);
    ...
    // ---- Case B: ragged + padding invariance ----
    {
        ...
        layer.forward(x, T, pe, pos_len, /*valid_len*/v0, ref0);
        std::vector<float> ref1;
        layer.forward(x, T, pe, pos_len, /*valid_len*/v1, ref1);
        ...
        // item0 (full) must be UNCHANGED vs the full scalar ref (padding
        // invariance): the shorter neighbor must not perturb it.
        bool b0 = pktest::compare(item0, ref0, "caseB_item0", 1e-3f, 1e-3f);

        // item1's first v1 valid query rows must match ref1's first v1 rows.
